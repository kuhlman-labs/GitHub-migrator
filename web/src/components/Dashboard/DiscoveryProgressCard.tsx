import { ProgressBar, Label, Button } from '@primer/react';
import { SyncIcon, CheckCircleIcon, AlertIcon, XCircleIcon, XIcon, StopIcon } from '@primer/octicons-react';
...
export function DiscoveryProgressCard({ progress, onDismiss, onCancel, isCancelling = false }: DiscoveryProgressCardProps) {
  ...
  const statusIcon = isFailed 
    ...
    : isCancelled
      ? <span style={{ color: 'var(--fgColor-danger)' }}><StopIcon size={16} /></span>
      : isComplete 
  ...
  return (
    <div className="p-4 rounded-lg border" style={cardStyle}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {isInProgress && !isCancellingPhase && onCancel && (
            <Button 
              size="small" 
              variant="danger"
              onClick={onCancel}
      ...
      {isCancelled && (
        <div className="text-sm">
          <p style={{ color: 'var(--fgColor-muted)' }}>
            Discovery was cancelled after processing {progress.processed_repos} of {progress.total_repos} repositories
          </p>
          {progress.completed_at && (
            <p className="mt-1" style={{ color: 'var(--fgColor-muted)' }}>
              Cancelled {new Date(progress.completed_at).toLocaleString()}
            </p>
          )}
        </div>
      )}
